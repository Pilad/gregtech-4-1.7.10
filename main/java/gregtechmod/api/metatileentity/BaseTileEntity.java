			...
			for (Object tObject : getWorld().playerEntities) {
				if (tObject instanceof EntityPlayerMP) {
					...
					if (Math.abs(tPlayer.posX - getXCoord()) < 256 && Math.abs(tPlayer.posZ - getZCoord()) < 256) {
						/** Need New packet **/
						//tPlayer.playerNetServerHandler.sendPacket(new Packet(getXCoord(), getYCoord(), getZCoord(), getWorld().getBlock(xCoord, yCoord, zCoord), aID, aValue));
					}
				} else {
					getWorld().addBlockEvent(getXCoord(), getYCoord(), getZCoord(), getWorld().getBlock(0, 0, 0), aID, aValue);
					break;
				}
			}
		} catch(Throwable e) {
