	    ...
	    if (getBaseMetaTileEntity().isServerSide()) {
	    	if (getBaseMetaTileEntity().getTimer()%256==0&&(getBaseMetaTileEntity().getWorld().isThundering()||(getBaseMetaTileEntity().getWorld().isRaining()&&getBaseMetaTileEntity().getRandomNumber(10)==0))) {
	        	...
	        	for (int i = getBaseMetaTileEntity().getYCoord() + 1; i < getBaseMetaTileEntity().getWorld().getHeight()-1; i++) {
	        		if (rodvalid && getBaseMetaTileEntity().getWorld().getBlock(0, i-getBaseMetaTileEntity().getYCoord(), 0) == Block.getBlockFromItem(GT_ModHandler.getIC2Item("ironFence", 1).getItem())) {
	        			rodvalue++;
	        		} else {
	        			rodvalid = false;
	        			if ( getBaseMetaTileEntity().getWorld().getBlock(0, i-getBaseMetaTileEntity().getYCoord(), 0) != Blocks.air) {
	        				rodvalue=0;
	        				break;
	        			}
	        		}
