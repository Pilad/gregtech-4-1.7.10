...
public class GT_MetaTileEntity_Grinder extends GT_MetaTileEntity_BasicTank {
    ...
    private boolean checkMachine() {
    	...
    	for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++) for (int k = -1; k < 2; k++) {
    		if (i!=0||j!=0||k!=0) {
    			if (getBaseMetaTileEntity().getWorld().getBlock(-xDir+i, -yDir+j, -zDir+k) != GregTech_API.sBlockList[0]) return false;
            	if (getBaseMetaTileEntity().getMetaIDOffset(-xDir+i, -yDir+j, -zDir+k) != (j==0?14:13)) return false;
    		} else {
    			if (getBaseMetaTileEntity().getWorld().getBlock(-xDir+i, -yDir+j, -zDir+k) != Blocks.water) return false;
    		}
    	}
    	return true;
    }
