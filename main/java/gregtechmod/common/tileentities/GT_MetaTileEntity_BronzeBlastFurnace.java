...
public class GT_MetaTileEntity_BronzeBlastFurnace extends MetaTileEntity {
    ...
    private boolean checkMachine() {
    	...
    	for (int i = -1; i < 2; i++) for (int j = -1; j < 3; j++) for (int k = -1; k < 2; k++) if (xDir+i != 0 || j != 0 || zDir+k != 0) {
    		if (i!=0||j==-1||k!=0) {
    			if (getBaseMetaTileEntity().getWorld().getBlock(xDir+i, j, zDir+k) != GregTech_API.sBlockList[4] || getBaseMetaTileEntity().getMetaIDOffset(xDir+i, j, zDir+k) != 13) return false;
    		} else {
    			if (getBaseMetaTileEntity().getWorld().getBlock(xDir+i, j, zDir+k) != Blocks.flowing_lava && !getBaseMetaTileEntity().getAirOffset(xDir+i, j, zDir+k)) return false;
    		}
    	}
    	return true;
    }
